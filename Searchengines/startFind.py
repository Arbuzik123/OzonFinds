...
import os
import time
from Searchengines.OzonSearch import SearchOzon
chromedriver_autoinstaller.install()
async def OzonFind(cat):
    print(cat)
    ...
    for idx, chunk in enumerate(chunks):
        time.sleep(1)
        path = rf"UpdatesOzonFinds/" + cat.replace(".xlsx", "") + f"_{idx + 1}.xlsx"
    ...
    for i in range(num_chunks):
        ...
        p = Process(target=SearchOzon, args=(i, path, lock, window_PosX, window_PosY, positions.pop(0)))
        processes.append(p)
    ...
    output_path = os.path.join('UpdateResultOzonFinds', rf'{cat.replace(".xlsx", "")}.xlsx')
